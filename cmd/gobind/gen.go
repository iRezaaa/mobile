...
import (
	...
	"github.com/iRezaaa/mobile/bind"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/iRezaaa/mobile/bind")
			if err != nil {
				errorf(`"github.com/iRezaaa/mobile/bind" is not found; run go get github.com/iRezaaa/mobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/iRezaaa/mobile directory.
			...
			javaDir, err := packageDir("github.com/iRezaaa/mobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/iRezaaa/mobile/bind/objc")
			if err != nil {
