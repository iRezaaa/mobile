...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/iRezaaa/mobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command(goBin(), "list", "-f", "{{.Stale}}", "github.com/iRezaaa/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command(goBin(), "list", "-f", "{{.Dir}}", "github.com/iRezaaa/mobile/app").CombinedOutput()
	if err != nil {
