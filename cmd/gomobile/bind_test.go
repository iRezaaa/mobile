...
func TestImportPackagesPathCleaning(t *testing.T) {
	...
	slashPath := "github.com/iRezaaa/mobile/example/bind/hello/"
...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/iRezaaa/mobile/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
WORK=$WORK
GOOS=android CGO_ENABLED=1 gobind -lang=go,java -outdir=$WORK{{if .JavaPkg}} -javapkg={{.JavaPkg}}{{end}} github.com/iRezaaa/mobile/asset
...
WORK=$WORK
GOOS=darwin CGO_ENABLED=1 gobind -lang=go,objc -outdir=$WORK -tags=ios{{if .Prefix}} -prefix={{.Prefix}}{{end}} github.com/iRezaaa/mobile/asset
